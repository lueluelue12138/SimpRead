...
- **定义：** 并发是指两个或多个任务在同一时间段内执行，但不一定同时进行。在单处理器系统中，这些任务通过时间片轮转的方式交替执行。在多处理器系统中，任务可以真正同时执行在不同的处理器上。
- **特点：** 并发强调多个任务能够在相同的时间段内执行，但不一定要求同时执行。任务之间可能会共享资源，而需要通过同步机制来避免冲突。

**并行（parallel）是同一时间动手做（doing）多件事情的能力**

- **定义：** 并行是指两个或多个任务在同一时刻执行，真正同时进行。这通常发生在多核处理器或多处理器系统中，每个任务分配给不同的处理器核心，同时执行。
- **特点：** 并行强调同时执行，各任务之间可以独立运行，不会相互影响。并行性通常需要硬件支持，如多核处理器。
...
- 在AQS中维护了一个使用了volatile修饰的state属性来表示资源的状态，0表示无锁，1表示有锁,如果持有这把锁的人再次持有这把锁，那么state就会+1,代表**锁重入**
...
`Semaphore` 类是 Java 中并发编程的一个工具类，它提供了一种计数信号量的实现。信号量是一种用于控制多个线程之间访问共享资源的并发工具，它可以控制同时访问某个特定资源的线程数量。

`Semaphore` 主要有两个目的：

1. **控制并发线程数：** 通过设置信号量的许可数量，可以限制同时执行某个代码块的线程数量。
2. **保护共享资源：** 信号量可以用于保护共享资源，确保同时只有有限数量的线程可以访问共享资源。

#### 主要方法：

1. **`Semaphore(int permits)`：** 创建一个具有给定许可数的 Semaphore。

```java
Semaphore semaphore = new Semaphore(5); // 允许同时有5个线程访问
```

**`void acquire()`：** 获取一个许可，如果没有许可则阻塞等待。
...
```
int waitingThreads = semaphore.getQueueLength();
```

#### 例子：

假设有一个资源池，同时最多只能有3个线程访问资源，可以使用 `Semaphore` 来实现：
...
在上述例子中，`Semaphore` 限制了同时访问 `accessResource` 方法的线程数量，确保最多只有3个线程可以同时访问资源。这有助于保护共享资源免受并发访问的问题。
