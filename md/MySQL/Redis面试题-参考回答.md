...
布隆过滤器是通过事先将一些关键字添加到过滤器中进行初始化的。这些关键字在初始化布隆过滤器时会被添加到过滤器中。这样，这些关键字就被认为是存在于布隆过滤器中的。当实际进行搜索时，通过布隆过滤器可以判断某个关键字可能存在于缓存中，从而避免不必要的缓存查询。在实际应用中，可以根据业务特点选择添加哪些关键字到布隆过滤器中。

#### **面试官：什么是缓存击穿 ? 怎么解决 ?(单个key**)
